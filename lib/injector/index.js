...
module.exports = function (hexo) {
  ...
  if (themeConfig.site_meta.cdn) {
    ...
    const rawLocales = fs.readFileSync(resolve(`../../data/${themeConfig.site_meta.cdn}.yml`));
    ...
    if (themeConfig.gitalk && themeConfig.gitalk.enable) {
      for (let cdn of locales.plugins.gitalk) {
        hexo.extend.injector.register('head_ned', cdn);
      }
    }

    if (themeConfig.valine && themeConfig.valine.enable) {
      for (let cdn of locales.plugins.valine) {
        hexo.extend.injector.register('head_ned', cdn);
      }
    }

    if (themeConfig.twikoo && themeConfig.twikoo.enable) {
      for (let cdn of locales.plugins.twikoo) {
        hexo.extend.injector.register('head_ned', cdn);
      }
    }

    if (themeConfig.waline && themeConfig.waline.enable) {
      for (let cdn of locales.plugins.waline) {
        hexo.extend.injector.register('head_ned', cdn);
      }
    }
  }
