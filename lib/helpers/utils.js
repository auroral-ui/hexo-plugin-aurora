...
exports.categorize = function (key, data) {
  ...
  for (var item of data) {
    ...
    if (!keyArr.has(dataKey)) {
      keyArr.add(dataKey);
      newData = { ...newData, [dataKey]: [item] };
    } else {
      newData[dataKey].push(item);
    }
  }
...
exports.avatarWall = function (data) {
  ...
  function addingItem(source) {
    for (var item of source) {
      if (sizeCount === requiredSize) return;

      if (!newData[arrKey]) {
        newData.push([item]);
        sizeCount++;
      } else if (newData[arrKey].length < 2) {
        newData[arrKey].push(item);
        sizeCount++;
        arrKey++;
      }
    }
