...
function authorMapper(author, configs) {
  ...
  if (typeof author === 'string' && configAuthors[author]) {
    ...
    return authorAttributes({
      ...
      slug: 'blog-author',
      avatar: configs.theme_config.site.avatar || configs.theme_config.site.logo,
...
function authorAttributes(author) {
  return {
    name: author.name,
    slug: author.slug || String(author.name).toLocaleLowerCase().replace(/[\s]+/g, '-'),
...
function pageMapper(page) {
  ...
  return {
    ...
    data: JSON.stringify({
      ...
      categoryMode: page.categoryMode,
      avatarWall: pageData ? avatarWall(pageData) : [],
      data: !!page.categoryMode
        ? pageData.length > 0
          ? categorize('label', pageData)
          : {}
        : pageData
    })
  };
